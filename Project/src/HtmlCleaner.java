...
public class HtmlCleaner {
	...
	/**
	 * Removes all HTML tags and certain block elements from the provided text. The block elements
	 * removed include: head, style, script, noscript, and svg.
	 *
	...
	/**
	 * Removes comments and certain block elements from the provided html. The block elements removed
	 * include: head, style, script, noscript, and svg.
	 *
	...
	/**
	 * Replaces all HTML entities with an empty string. For example, "2010&ndash;2012" will become
	 * "20102012".
	 *
	...
	/**
	 * Replaces all HTML tags with an empty string. For example, "A<b>B</b>C" will become "ABC".
	 *
	 * @param html text including HTML tags to remove
	 * @return text without any HTML tags
	 */
	public static String stripTags(String html) {
		String regex = "(?msi)<\\w*?(\\s?)+.*?>|<\\/\\w+?>";
		Pattern pattern = Pattern.compile(regex);
		Matcher matcher = pattern.matcher(html);
		return matcher.replaceAll("");
	}
	...
	/**
	 * Replaces all HTML comments with a single space. For example, "A<!-- B -->C" will become "A C".
	 *
	...
	/**
	 * Replaces everything between the element tags and the element tags themselves with a single space.
	 ...
	 * </pre>
	 *
	 * If removing the "style" element, all of the above code will be removed, and replaced with a
	 * single space.
	 *
	...
	public static String stripElement(String html, String name) {
		String regex = "(?msi)<" + name + "\\b.+?" + name + "\\s*?>";
		Pattern pattern = Pattern.compile(regex);
		Matcher matcher = pattern.matcher(html);
		return matcher.replaceAll(" ");
	}
